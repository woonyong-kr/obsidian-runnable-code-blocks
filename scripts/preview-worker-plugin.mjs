...
async function runtime() {
  bundle ??= Promise.all([
    esbuild.build({ entryPoints: [fileURLToPath(new URL('../src/preview-worker/frame.mts', import.meta.url))], bundle: true, format: 'iife', globalName: '__RCB_PREVIEW__', minify: true, platform: 'browser', target: 'es2022', write: false }),
    readFile(require.resolve('@ampproject/worker-dom/dist/worker/worker.js'), 'utf8'),
    esbuild.build({ entryPoints: [fileURLToPath(new URL('../src/preview-worker/canvas.mts', import.meta.url))], bundle: true, format: 'iife', minify: true, platform: 'browser', target: 'es2022', write: false })
